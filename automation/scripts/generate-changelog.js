...
async function generateChangelog() {
  ...
  const addedLinks = Object.values(newMods).map(metadata => formatModLink(metadata));
  const removedLinks = Object.values(removedMods).map(metadata =>
    formatModLink(metadata)
  );
  ...
  const rawCommits = (
    await $`git log ${CONFIG.cutoffCommitHash}..${CONFIG.referenceCommit} --pretty=format:"%s \`%an\`"`.text()
  )
    .split("\n")
    .filter(Boolean);
