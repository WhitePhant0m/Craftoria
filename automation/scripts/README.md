...
- **Auto Cutoff Detection**: Finds last version bump commit automatically(having tags would better)
...
Only `feat` and `fix` commits appear in the changelog. To exclude a commit, use `dev` or `debug` scope.
